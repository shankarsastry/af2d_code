...
double PSLG::segment_length(int i) const {
    ...
    return vertices[s.p].distance(vertices[s.q]);
}

Point2D PSLG::point_on_segment(int i, double x) const {
    const auto& s = segments[i];
    const auto& pa = vertices[s.p];
    const auto& pb = vertices[s.q];
...
bool PSLG::is_adjacent(int seg_i, int feature_seg) const {
    const auto& si = segments[seg_i];
    const auto& sf = segments[feature_seg];
